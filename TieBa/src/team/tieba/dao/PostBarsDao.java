...
public class PostBarsDao {
	...
	public List<Posts> getPopularDynamic(String parent_id) {
		...
		String sql = "select * from posts where rownum < 5 and parent_id='"
				+ parent_id + "' order by sendtime desc";
		...
		try {
			...
			postsList = new QueryRunner().query(conn, sql,
	...
	public Map<String, Object> getPostsAndBarInf(String bname) {
		...
		try {
			...
			bar = new QueryRunner().query(conn, barsql,
					new BeanHandler<PostBars>(PostBars.class));
			maps.put("bar", bar);

			follows = new QueryRunner().query(conn, followsql,
	...
	public List<Posts> search(String searchName) {
		...
		Connection conn = null;
		String sql = "select * from posts where content like '%" + searchName
				+ "%' and parent_id='0'";
	...
	public boolean attention(Follow follow) {
		...
		Object[] param = { follow.getUname(), follow.getBname() };
	...
	public boolean subAttention(Follow follow) {
		...
		Connection conn = null;
		String sql = "delete follow where uname=? and bname=?";
	...
	public List<PostBars> getMyPostBars(String uname) {
		...
		String sql = "select p.* from follow f, postbars p where p.bname=f.bname and f.uname='"
				+ uname + "'";

		try {

			conn = OracleDBUtil.getODBconn();
			postbars = new QueryRunner().query(conn, sql,
					new BeanListHandler<PostBars>(PostBars.class));
